...
# Set paths.
declare YUICOMPRESSOR=$ESDOC_JSPLUGIN_HOME"/sh/yuicompressor-2.4.7.jar"
declare SRC=$ESDOC_JSPLUGIN_HOME
declare BIN=$ESDOC_JSPLUGIN_HOME/bin
declare ESDOC_JSPLUGIN_TMP=$ESDOC_JSPLUGIN_HOME/tmp
declare ESDOC_COMPARATOR=$ESDOC_JSPLUGIN_HOME/../esdoc-web-compare

# Initialise temporary directory.
mkdir -p $ESDOC_JSPLUGIN_TMP
...
_reset_output_folders()
{
    rm -rf $BIN/latest
    rm -rf $BIN/$VERSION
    rm -rf $ESDOC_COMPARATOR'/media'
    mkdir -p $BIN/latest/images
    mkdir -p $BIN/$VERSION/images
    mkdir -p $ESDOC_COMPARATOR'/media/images'
}
...
{
    mkdir -p $ESDOC_JSPLUGIN_TMP
    mkdir -p $ESDOC_JSPLUGIN_TMP/api
    mkdir -p $ESDOC_JSPLUGIN_TMP/core
    mkdir -p $ESDOC_JSPLUGIN_TMP/ext
    mkdir -p $ESDOC_JSPLUGIN_TMP/media/images
    mkdir -p $ESDOC_JSPLUGIN_TMP/tools
    mkdir -p $ESDOC_JSPLUGIN_TMP/tools/comparator/c1
    mkdir -p $ESDOC_JSPLUGIN_TMP/tools/comparator/views
    mkdir -p $ESDOC_JSPLUGIN_TMP/tools/search
    mkdir -p $ESDOC_JSPLUGIN_TMP/tools/viewer
    mkdir -p $ESDOC_JSPLUGIN_TMP/utils
}
...
_minimize()
{
    ...
    do
        ...
        declare dest="$ESDOC_JSPLUGIN_TMP/$f"
